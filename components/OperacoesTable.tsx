            ...
            {displayed.map((row) => {
              ...
              return (
                ...
                >
                  ...
                  <td style={{ color: 'var(--text-muted)', fontSize: 12 }}>{row.seq}</td>
                  ...
                  <td className="mono" style={{ fontSize: 13 }}>{fmtRS(row.capitalAcumulado)}</td>
                  <td className="mono" style={{ fontSize: 13, color: 'var(--text-muted)' }}>{fmtRS(row.topo)}</td>
                  <td style={{
                    fontWeight: 700,
                    fontSize: 13,
        ...
        <tbody>
          {displayed.map((op, i) => (
            <tr key={op.id}>
              ...
              <td>{formatDate(op.data)}</td>
              <td style={{ color: 'var(--text-muted)', fontSize: 11 }}>{op.dia_semana}</td>
              <td><span style={{ fontWeight: 700 }}>{op.ativo}</span></td>
              <td style={{ color: op.tipo === 'Compra' ? 'var(--gain)' : 'var(--loss)' }}>{op.tipo}</td>
              <td className="mono">{op.pe?.toLocaleString('pt-BR')}</td>
              <td className="mono">{op.stop?.toLocaleString('pt-BR')}</td>
